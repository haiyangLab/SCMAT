...
def parse_model_config(path):
    """
    Parses the model layer configuration file
    Args:
        path: model config path
    Returns: returns module definition
    """
    file = open(path, 'r')
...
def parse_data_config(path):
    """
    Parse the data configure file
    Args:
        path: data config file path
...
def weights_init_norm(model):
    ...
    if classname.find("Conv") != -1:
        nn.init.normal_(model.weight.data, 0.0, 0.0)
    elif classname.find("BatchNorm2d") != -1:
        nn.init.normal_(model.weight.data, 1.0, 0.02)
        nn.init.normal_(model.bias.data, 0.0)
